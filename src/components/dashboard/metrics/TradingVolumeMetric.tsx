...
interface TradingVolumeMetricProps {
  assetName?: string;
}

export function TradingVolumeMetric({ assetName }: TradingVolumeMetricProps) {
  ...
  React.useEffect(() => {
    ...
    root.style.setProperty('--color-volume', chartConfig.volume.theme.dark);
  }, []);

  if (isLoading || !metrics.volume) {
    return (
      ...
      <Card className="border-gold/20 bg-white/70 dark:bg-charcoal-light/70 dark:border-gold/30">
        ...
        <CardContent>
          <div className="text-sm text-red-600 dark:text-red-400">
            Failed to load trading volume data
          </div>
        </CardContent>
      </Card>
    );
  ...
  const { data, currentValue, percentChange24h } = metrics.volume;
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={data} margin={{ top: 5, right: 5, bottom: 5, left: 5 }}>
                ...
                <Line
                  ...
                  strokeWidth={2}
                  dot={false}
                  activeDot={{ stroke: "var(--color-volume)", strokeWidth: 2, r: 6 }}
                />
              </LineChart>
            </ResponsiveContainer>
          </ChartContainer>
