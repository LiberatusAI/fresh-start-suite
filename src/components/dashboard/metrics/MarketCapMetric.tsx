...
interface MarketCapMetricProps {
  assetName?: string;
}

export function MarketCapMetric({ assetName }: MarketCapMetricProps) {
  ...
  React.useEffect(() => {
    ...
    root.style.setProperty('--color-marketcap', chartConfig.marketcap.theme.dark);
  }, []);

  if (isLoading || !metrics.marketcap) {
    return (
  ...
  const { data, currentValue, percentChange24h } = metrics.marketcap;
